...
三个设计决策：
    ...
    D2 引用判定是**代理指标**，不是真答案：用字符 4-gram 重叠率当"这块资料
       ...
       **实测**（2026-09-13，同一条 query「SKU-10001 这耳机口碑怎么样 有什么缺点」
       前后跑两次真实 LLM）：两次的召回**完全相同**（5 个 chunk_id 与 rrf_score
       逐一对齐，检索路径是确定的），但引用块数一次是 1、一次是 3：

           跑次    各块重叠率                       判为引用
           第一次  0.1154 0.0333 0.0909 0.2364 ...  1 块
           第二次  0.2500 0.2333 0.1023 0.2182 ...  3 块

       差异全部来自**措辞**：第二次的回复把块里的「（312 次）（198 次）」这类
       括注原样抄了进去，第一次改写成"主要夸降噪强、续航久"这种自然语言，
       字面就对不上了。所以这个数度量的是"模型**抄了多少字面**"，不是
       "用了多少资料"——它**会低报**，且低报程度随措辞波动（同一 query 跨轮
       就能差 3 倍）。分块少了反而稳：同期的「冲锋衣怎么洗」轮只有 1 块真正
       被用，两次都准确命中那一块、其余全 0。
       对策不是去调那个常数，而是：每块原始重叠率都落在记录里（hits[].cite_overlap），
       阈值只是个便利标记。要重新划线，拿已有日志离线重算即可，不必重跑 LLM。
       **别把单轮的这个数当成质量结论——它的方差比绝对值更值得注意。**
